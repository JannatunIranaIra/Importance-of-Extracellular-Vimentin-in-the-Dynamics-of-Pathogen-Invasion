...
for (j in 1:length(all1)){
  #parameter values
  parameters <- list(lambda1 = 2.5, #loop
                     b = 0.061,
                     delta1 = 0.01,
                     delta2 = 10^4,
                     pi = 5, 
  ...
  # cat("A =", A_coeff, "B =", B_coeff, "C =", C_coeff, "D =", D_coeff, "\n")
  ...
  for (j1 in 1:length(I_star)) {
    ...
    if (any(is.nan(P_star) | is.nan(V_star) | is.nan(Pv_star) | is.nan(equi_I))) {
      ...
      if (equi_I>= 0 & Pv_star >= 0 & P_star>= 0 & V_star>=0) {
        #if (length((equi_I))>2) {print("HELLO!") }
        count <- count +1
...
critical_point <- c(5e-5,     # stable ife
                    1.7e-4,   #stable ife near switch point 
                    1.9e-4,     #unstable ife near switch point
                    0.02,
                    5.8,
                    5.9,
                    10)     #unstable ife 
...
for (j in 1:length(critical_point)){
  #parameter values
  parameters <- list(lambda1 =2.5e-1, 
                     b = 0.061,
                     delta1 = 0.01,
                     delta2 = 0.05,
                     pi = 2, 
