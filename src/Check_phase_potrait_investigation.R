...
critical_point <- c(5e-4,     #1 stable equi -> 0
                    0.0019,   #switch (ptential critical point) 
                    0.02,     #2 unstable equi with 2 complex eigenvalues
                    0.05)     #2 unstable equi with 0 complex eigenvalues 
    ...
    if (any(is.nan(P_star) | is.nan(V_star) | is.nan(Pv_star) | is.nan(equi_I))) {
      ...
      if (equi_I>= 0 & Pv_star >= 0 & P_star>= 0 & V_star>=0) {
        ...
        if (equi_I == 0){
          ...
          agg_jacobian = matrix(c(-parameters$rho1, 0, 0, (parameters$delta1+parameters$delta2+parameters$k*parameters$kc)*parameters$pi,
                                  ...
                                  0,0, parameters$sigma1*(parameters$kc), parameters$b-parameters$delta1-parameters$delta2-parameters$k*parameters$kc),
          ...
          agg_jacobian = matrix(c(-parameters$lambda1*V_star-parameters$rho1, -parameters$lambda1*P_star, 0, (parameters$delta1+parameters$delta2+parameters$k*parameters$kc)*parameters$pi,
                                  ...
                                  0,0, parameters$sigma1*(parameters$kc-equi_ie[count]), -parameters$sigma1*Pv_star+parameters$b-parameters$delta1-parameters$delta2-parameters$k*parameters$kc),
                                nrow = 4, ncol = 4, byrow = TRUE )
        }
...
for (j in 1:length(critical_point)){
  ...
  out <- ode(y = state, times = times, func = Model, parms = parameters, atol = 1e-8, rtol = 1e-12, hmin = 1e-6, method = "lsoda")
...
plot_ly(out_df, x = ~P, y = ~V, z = ~Pv, type = "scatter3d", mode = "lines",
        line = list(width = 3, color = "blue")) %>%
  layout(scene = list(
    xaxis = list(title = "Pathogen (P)"),
    yaxis = list(title = "Vimentin (V)"),
...
plot_ly(out_df, x = ~log10(P), y = ~log10(V), z = ~log10(I), type = "scatter3d", mode = "lines",
        line = list(width = 3, color = "blue")) %>%
  layout(scene = list(
    xaxis = list(title = "Pathogen (P)"),
    yaxis = list(title = "Vimentin (V)"),
...
plot_ly(out_df, x = ~V, y = ~Pv, z = ~I, type = "scatter3d", mode = "lines",
        line = list(width = 3, color = "blue")) %>%
  layout(scene = list(
    xaxis = list(title = "Vimentin (V)"),
    yaxis = list(title = "P-V complex (Pv)"),
